        ...
        <div className="glass-card p-8 md:p-12 border-primary/30 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              <span className="badge-glass border-accent/40 text-accent">Coming Soon</span>
            </div>
            ...
            <p className="text-muted-foreground max-w-2xl mb-2">
              Expected <strong className="text-foreground">October–November 2026</strong>. Official date coming soon — join early to get speaker slots, sponsorships, and community access.
            </p>

            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="grid grid-cols-4 gap-3 md:gap-4 max-w-xl mt-8">
              {boxes.map((b) => (
                <div key={b.label} className="glass-card p-4 text-center border-primary/20">
                  <p className="text-2xl md:text-4xl font-display font-bold gradient-text tabular-nums">
                    {String(b.v).padStart(2, "0")}
                  </p>
                  <p className="text-[10px] uppercase tracking-widest text-muted-foreground mt-1">{b.label}</p>
                </div>
              ))}
            </motion.div>
            ...
            <div className="mt-10 pt-6 border-t border-border/40">
              <p className="text-xs text-muted-foreground mb-3 uppercase tracking-widest">Community Highlights</p>
              <div className="flex flex-wrap gap-2">
                {GDGS.map((g) => (
                  <span key={g} className="badge-glass">{g}</span>
                ))}
              </div>
            </div>
          </div>
